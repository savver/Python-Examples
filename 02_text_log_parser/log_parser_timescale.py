...
print("print log filename:")
infile = input()    #raw_input
if infile.find(".txt") < 0:
    infile += ".txt"
handle = open(infile)
fout_ts1 = open(infile + "__sens1_out.txt", 'w')
fout_ts2 = open(infile + "__sens2_out.txt", 'w')
...
for line in handle:
    start = line.find("<00002>")
    if start > 0:
        space_idx = line.find(' ')
        line_time = year_str + line[0:space_idx] + us2ms_str
        ...
        sep = len(line) - 1
        line1 = line[start+len("<00002>"):sep]
        line1 = line1.lstrip()
        space_idx = line1.find(' ')
        line2 = line1[space_idx:len(line1)-1]
...
plt.title('sensors data' + ' (' + infile + ')')
plt.xlabel('x, ms')
plt.ylabel('0.001 Amps * val')
